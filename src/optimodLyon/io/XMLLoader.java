...
public class XMLLoader
{
     ...
     */
    public DeliveryPlan loadDeliveryPlan(final String path) throws MalformedXMLException, FileNotFoundException, SecurityException, IOException
    {
        ...
        try
        {
            ...
            {
                throw new MalformedXMLException(String.format("L'attribut de la date de départ n'est pas présente dans le fichier %s", path));
            }

            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("HH:mm:ss");
            LocalDateTime departureTime = LocalDateTime.parse(departureTimeAttribute, formatter);
            ...
            if (planningRequestNode.getNodeType() == Node.ELEMENT_NODE)
            {
                ...
                for (int i = 0; i < requestsNodeList.getLength(); ++i)
                {
                    ...
                    int deliveryAddress = Integer.parseInt(deliveryAddressAttribute);
                    int pickupAddress = Integer.parseInt(pickupAddressAttribute);
